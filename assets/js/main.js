...
// auto tour
var data1 = [];
var play, nametour, autotour;
var ID=0, result=0, time =0;
	
window.addEventListener("load", function(event){
	console.log('load');
    krpano = document.getElementById("panorama"); 
	get_nametour();

});

function autotour(){
    krpano.set("events[continue].onidle",null);
	var scenename = krpano.get("xml.scene");
	if ( ID == 0 ){
        n = 1;
	}else{
        n = ID-1;
		krpano.call("getlooktodistance(result," + data1[ID].hlookat + "," + data1[ID].vlookat + ",view.hlookat, view.vlookat)");
    }
	if ( ( data1[ID].scene != data1[n].scene ) || ID == 0 || data1[ID].scene != scenename ){
        krpano.call("loadscene("+ data1[ID].scene +", null, MERGE , BLEND(1,5))")
		krpano.call("getlooktodistance(result," + data1[ID].hlookat + "," + data1[ID].vlookat + ",0,0)");
		//console.log('ID: '+ID);
		//console.log('scene ID: '+data1[ID].scene);
    }
	if ( ID == data1.length - 1 || data1[ID].scene != data1[ID+1].scene ){	
        krpano.call(" wait(1);lookto("+ data1[ID].hlookat +"," + data1[ID].vlookat + ","+ data1[ID].fov +",smooth(23,23,23));lookto("+ data1[ID].hlookat +"," + data1[ID].vlookat + ","+ data1[ID].fov +",smooth(23,23,23))");
        //console.log('ID2: '+ID);
        //console.log('scene ID2: '+data1[ID].scene);
    }else{
        krpano.call(" wait(1);lookto("+ data1[ID].hlookat +"," + data1[ID].vlookat + ","+ data1[ID].fov +",smooth(23,23,23));lookto("+ data1[ID].hlookat +"," + data1[ID].vlookat + ","+ data1[ID].fov +",smooth(23,23,23));wait(2);lookto("+ data1[ID].hlookat +"," + data1[ID].vlookat+ ","+ data1[ID].fov +",smooth(23,23,23))");
        //console.log('ID3: '+ID);
        //console.log('scene ID3: '+data1[ID].scene);	
    }
	result = krpano.get("result");
	if (ID == data1.length - 1 ){
		autoStop();
        ID=0;
		time = result /23 + 2 + 1.5 + Math.abs(data1[data1.length - 2].fov - 120)*2/23 +2;
		//console.log('time: '+time);
		//time = result /100 + 2 + 1.5 + Math.abs(data1[data1.length - 2].fov - 120)*2/100 +2;
    }else{
        ID++;
		time = result /23 + 2 + 1.5 + Math.abs(data1[ID-1].fov - 120)*2/23 +1;
		//console.log('time: '+time);
		//time = result /100 + 2 + 1.5 + Math.abs(data1[ID-1].fov - 120)*2/100 +1;
    }
	play = setTimeout(function(){ autotour();},time*700);
	
}

// Function láº¥y data cho tá»«ng tour
function get(){
	console.log('get');
    $.get( "starglobal/modules/autotour/php/get_autotour.php",{'nametour':nametour},
    function( data ) {
       data1 = JSON.parse(data); //chuyen du lieu tu text sang array
    });
}

// Function láº¥y nametour
function get_nametour(){
	console.log('get-nametour');
    $.get( "starglobal/modules/autotour/php/nametour_autotour.php",
    function (data) {
...
// Function thĂªm vĂ o droplist
function addToList() {
	console.log('addtolist');
    for (ID = 0; ID< data1.length; ID++){
        var x = document.getElementById("Listbox");
        var option = document.createElement("option");
        option.text = data1[ID].nametour;
        option.value = data1[ID].nametour;
        x.add(option);
    }
    ID = 0;
}
// Function nĂºt Play ( Báº¯t Ä‘áº§u)
function replayautotour(){
console.log('replayautotour');
	clearTimeout(stop);
    document.getElementById("stop").style.display  ="block"
    document.getElementById("play").style.display  ="none"
    nametour = document.getElementById("Listbox").value;
    get();
    setTimeout(() => {
        krpano.call("oninterrupt(js(pause()))");
    	autotour();
    	//Tá»± Ä‘á»™ng phĂ¡t
       // voicePancake.play();
    }, 600); 
...
// Function dá»«ng khi ngÆ°á»i dĂ¹ng tÆ°Æ¡ng tĂ¡c
function pause(){
console.log('pause');
	//voicePancake.pause();
	clearTimeout(play);
	krpano.set("idletime",1.5);
	krpano.set("events.name","continue");
	krpano.set("events[continue].keep",true);
	krpano.set("events[continue].onidle","oninterrupt(js(pause())); js('voicePancakePlay()'); js('autotour()')");

}

function pauseAutotour(){
console.log('pauseautotour');
	clearTimeout(play);
	krpano.set("idletime",1.5);
	//krpano.set("events.name","continue");
	krpano.set("events[continue].keep",false);
	krpano.set("events[continue].onidle","oninterrupt(js(pause()))");
	document.getElementById("stop").style.display  ="none"
    document.getElementById("play").style.display  ="block"
}


function voicePancakePlay(){
	console.log('voicepancakeplay');
	//setTimeout(() => {
        voicePancake.play();
    //}, 100);
}
